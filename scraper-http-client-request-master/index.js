...
const getClient = (limiter, promiseRetryOptions = null) => {
    ...
    return promiseRetry.withOptions({ priority }, (retry) => {
      ...
      return clients[currentStage](options)
        ...
        .catch((e) => {
          ...
          const errorMessage = rawMessage.length < 50 ? rawMessage : `server responded with code ${code}, try #${currentRetry}, SetCookie: ${cookie}`
  ...
  requestWrapper.setCookie = (cookies) => {
      cookie = cookies
  }
  requestWrapper.setUserAgents = (ua) => {
    uaser = ua;
  }
  requestWrapper.setDeafultUserAgents = () => {
    uaser = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.139 Safari/537.36"
  }
